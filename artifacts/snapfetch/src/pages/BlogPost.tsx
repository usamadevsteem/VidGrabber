...
export default function BlogPost() {
  ...
  const { data: post, isLoading } = useGetBlogPost(params.slug || "", {
    query: {
      enabled: !!params.slug,
      queryKey: getGetBlogPostQueryKey(params.slug || ""),
    },
  });

  if (isLoading) {
    return (
      <div className="py-32 flex justify-center bg-white min-h-[calc(100vh-4rem-400px)]">
  ...
  if (!post) {
    return (
      <div className="py-32 text-center bg-white min-h-[calc(100vh-4rem-400px)] text-gray-900">
        Post not found
      </div>
    );
  }
      ...
      <div className="container max-w-3xl px-4 pt-16">
        ...
        <div className="mb-12">
          <div className="flex items-center gap-4 text-sm text-gray-500 mb-6 flex-wrap font-medium">
            <span className="font-semibold text-primary bg-primary/10 px-3 py-1 rounded-full uppercase tracking-wider">
              {post.category}
            </span>
        ...
        <div
          className="prose prose-lg max-w-none text-gray-800 prose-headings:text-gray-900 prose-a:text-primary prose-a:no-underline hover:prose-a:underline prose-li:text-gray-700 prose-strong:text-gray-900"
